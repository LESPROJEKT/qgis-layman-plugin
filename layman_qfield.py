...
class URLMapping(Enum):
    SERVER1 = (
        "https://atlas2.kraj-lbc.cz",
        "https://atlas2.kraj-lbc.cz/qfcloud",
    )
    SERVER2 = ("https://watlas.lesprojekt.cz", "https://qfield.lesprojekt.cz")
...
class Qfield:
    ...
    def createQProject(self, name, description, private):
        ...
        response = self.utils.requestWrapper(
            "POST", url, payload=payload, files=None, emitErr=False
        )
        print(url)
    ...
    def convertQProject(self):
        ...
        cloud_convertor = CloudConverter(
            QgsProject.instance(), path, self.selectedLayers
        )
    ...
    def uploadQFiles(self, project_id, path):
        ...
        if len(layers) == 0:
            self.utils.showMessageSignal.emit(
                [["Nejsou vrstvy k exportu!", "No layers to export!"]], Qgis.Warning
            )
            return
        path = self.convertQProject()
        threading.Thread(
            target=lambda: self.postMultipleFiles(project_id, path)
        ).start()
    ...
    def getProjects(self):
        ...
        response = self.utils.requestWrapper(
            "GET", url, payload=None, files=None, emitErr=True
        )
        return response
    ...
    def updateProjectPermissions(self, project_id, username, permissions):
        ...
        response = self.utils.requestWrapper(
            "PUT", url, payload=json.dumps(permissions), files=None, emitErr=True
        )
        return response

    def updateQfieldCloudPermissions(
        self, access_rights_json, project_id, existing_users
    ):
        ...
        for role, users in access_rights_json["access_rights"].items():
            ...
            for username in users:
                ...
                if username in existing_users_set:
                    ...
                    response = self.utils.requestWrapper(
                        "PUT", url, payload=json.dumps(data), files=None, emitErr=True
                    )
    ...
    def postPermissionsForProject(self, project_id, role, username):
        payload = {"collaborator": username, "role": role}
        url = f"{self.URI}/api/v1/collaborators/{project_id}/"
        response = self.utils.requestWrapper(
            "POST", url, payload=payload, files=None, emitErr=False
        )
    ...
    def putPermissionsForProject(self, project_id, role, username):
        ...
        url = f"{self.URI}/api/v1/collaborators/{project_id}/{username}/"
        response = self.utils.requestWrapper(
            "PUT", url, payload=payload, files=None, emitErr=False
        )
        return response
    ...
    def patchPermissionsForProject(self, project_id, role, username):
        ...
        url = f"{self.URI}/api/v1/collaborators/{project_id}/{username}/"
        response = self.utils.requestWrapper(
            "PATCH", url, payload=payload, files=None, emitErr=False
        )
        return response

    def deletePermissionsForProject(self, project_id, username):
        data = json.dumps({"collaborator": username})
    ...
    def postProjectFile(self, project_id, filename):
        ...
        files = {"file": (filename, open(fullname, "rb"))}
        url = f"{self.URI}/api/v1/files/{project_id}/{filename}/"
        response = self.utils.requestWrapper(
            "POST", url, payload=None, files=files, emitErr=False
        )
        return response
    ...
    def deleteProjectFile(self, project_id, filename):
        ...
        url = f"{self.URI}/api/v1/files/{project_id}/{filename}/"
        response = self.utils.requestWrapper(
            "DELETE", url, payload=None, files=None, emitErr=False
        )
        print(response.content)
        return response
    ...
    def findLayersToPost(self, local_layers, server_layers):
        ...
        layers_to_upload = [
            ...
            if layer["title"] not in server_layer_names
        ]
    ...
    def findLayersToDelete(self, local_layers, server_layers):
        ...
        layers_to_delete = [
            ...
            if layer["name"] not in local_layer_titles
        ]
        layers_to_delete = [
            layer["name"]
            for layer in server_layers
    ...
    def findLayersToCheck(local_layers, server_layers):
        server_gpkg_layers = {
            layer["name"] for layer in server_layers if layer["name"].endswith(".gpkg")
        }
        gpkg_layers_to_check = [
            layer
            for layer in local_layers
            if layer.endswith(".gpkg") and layer in server_gpkg_layers
        ]
    ...
    def downloadProjectPackage(self, project_id):
        ...
        try:
            ...
            download_directory = tempfile.mkdtemp(
                prefix="qfield_", dir=tempfile.gettempdir()
            )
            ...
            for file_info in files:
                ...
                download_url = (
                    f"{self.URI}/api/v1/packages/{project_id}/latest/files/{filename}"
                )
                if filename.lower().endswith(
                    ...
                    local_path = os.path.join(
                        dcim_directory, os.path.basename(filename)
                    )
                else:
                    local_path = os.path.join(
                        download_directory, os.path.basename(filename)
                    )
    ...
    def qfieldPermissionsJunction(
        self, project_id, users_write, users_read, laymanUsername
    ):
