...
FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "dlg_currentComposition.ui")
)
...
class CurrentCompositionDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def setUi(self):    
        ...
        self.pushButton_setPermissions.clicked.connect(
            lambda: self.setStackWidget("permissions")
        )
        self.pushButton_back.clicked.connect(lambda: self.setStackWidget("main", False))  
        self.pushButton_editMeta.clicked.connect(
            lambda: self.setStackWidget("metadata")
        )
    ...
    def UpdateQfield(self):
        ...
        if self.layman.laymanUsername == self.layman.instance.getWorkspace():
            ...
            print("DEBUG: User owns workspace for QField export - setting full permissions")
        else:
        ...
        response = self.qfield.createQProject(
            ...
            self.layman.instance.getDescription(),
            permission,
        )
        res = response.json()       
        if response.status_code == 201:
            self.utils.showQgisBar(
                [" Projekt by úspěšně vytvořen.", " Project was successfully created."],
                Qgis.Success,
            )
        elif "code" in res and res["code"] == "project_already_exists":
            convertedProjectPath = self.qfield.convertQProject()            
            self.utils.showQgisBar(
                [" Aktualizuji project QField.", " Update QField project"], Qgis.Success
            )
            ...
            if self.utils.hasMatchingLayer(self.utils.getWmsOrWfsLayers(), qfieldFiles):
                self.utils.showQgisBar(
                    [
                        " Není zachován původní poskytoval dat pro Qfield.",
                        " Different layer provider.",
                    ],
                    Qgis.Warning,
                )
            ...
            layersToPost = self.qfield.findLayersToPost(
                self.layman.instance.getLayerList(), qfieldFiles
            )
            ...
            self.syncFiles(
                local_hashes, filesToCheck, layersToPost, layersToDelete, project_id
            )
    ...
    def qfieldThread(self):
        ...
        if self.layman.laymanUsername == self.layman.instance.getWorkspace():
            permissions = {"read": ["EVERYONE"], "write": ["EVERYONE"]}
            print("DEBUG: User owns workspace for QField thread - setting full permissions")
        else:
            permissions = self.layman.instance.getAllPermissions()
        permission = "true" if "EVERYONE" in permissions["read"] else "false"
        response = self.qfield.createQProject(
    ...
    def setVisibilityForCurrent(self, visible):           
        if self.layman.instance is None:
            ...
            return
        if (
            self.layman.laymanUsername != self.layman.instance.getWorkspace()
            and visible == True
        ):
            self.pushButton_editMeta.setEnabled(False)   
            self.pushButton_setPermissions.setEnabled(False)
    ...
    def refreshCurrentForm(self, layerAdded=None):
        ...
        try:
            (
                ...
                else self.setWindowTitle("Composition: " + composition["title"])
            )
        except:
            print("titulek nenačten")
        ...
        for layer in layersArr:  
            self.layerIds.append(
                [self.utils.removeUnacceptableChars(layer.name()), layer.id()]
            )
            ...
            if self.utils.removeUnacceptableChars(layer.name()) in layerList:
                ...
                item.setToolTip(
                    0,
                    self.tr(
                        "This layer is displayed and is part of the loaded composition."
                    ),
                )
            else:                
        ...
        for layer in notActive:            
            ...
            item.setToolTip(
                0,
                self.tr(
                    "This layer does not appear in the QGIS map window, but is included in the composition."
                ),
            )
            self.treeWidget_layers.addTopLevelItem(item)
    ...
    def checkAllLayers(self, checked):
        if checked:
            (
                self.checkBox_all.setText("Odebrat vše")
                if self.layman.locale == "cs"
        ...
        if not checked:
            (
                self.checkBox_all.setText("Vybrat vše")
                if self.layman.locale == "cs"
        ...
        while iterator.value():
            ...
            for layer in layersArr:                
                if (
                    ...
                    and cellServices.count() == 0
                ):
                    if (
                        isinstance(layer, QgsRasterLayer)
                        and "geoserver" in layer.dataProvider().dataSourceUri()
                    ):
                        ...
                        isinstance(layer, QgsRasterLayer)
                        and "geoserver" not in layer.dataProvider().dataSourceUri()
                    ):
                        ...
                        isinstance(layer, QgsVectorLayer)
                        and layer.dataProvider().name() == "WFS"
                        and urlServer not in layer.dataProvider().uri().uri()
                    ):
                        cellServices.addItems(["WFS"])
                    elif (
                        isinstance(layer, QgsVectorLayer)
                        and layer.dataProvider().name() == "WFS"
                        and urlServer in layer.dataProvider().uri().uri()
                    ):
                        cellServices.addItems(["WFS", "WMS"])

            if self.layman.instance.isLayerInComposition(
                self.utils.removeUnacceptableChars(item.text(0))
            ):
                if self.layman.locale == "cs":
                ...
                item.text(0)
                .replace(" (Smazána z projektu)", "")
                .replace(" (Removed from canvas)", "")
                in notActive
            ):
                if self.layman.locale == "cs":
                ...
                if self.utils.checkExistingLayers(
                    item.text(0), data
                ):  ##and self.utils.checkExistingLayer(item.text(0)):
                    if self.layman.locale == "cs":
                        cell.addItems(
                            ["Beze změny", "Přidat ze serveru", "Přidat a přepsat"]
                        )
                    else:
                        cell.addItems(
                            ["No change", "Add from server", "Add and overwrite"]
                        )
            ...
            if self.layman.instance.getServiceForLayer(item.text(0)) in (
                ["HSLayers.Layer.WMS", "WMS", "XYZ"]
            ):
        ...
        try:
            while iterator.value():
                ...
                if item.checkState(0) == 0 and (
                    self.treeWidget_layers.itemWidget(item, 2).currentText()
                    == "Add from server"
                    or self.treeWidget_layers.itemWidget(item, 2).currentText()
                    == "Přidat ze serveru"
                    or self.treeWidget_layers.itemWidget(item, 2).currentText()
                    == "Add and overwrite"
                    or self.treeWidget_layers.itemWidget(item, 2).currentText()
    ...
    def updateComposition(self, checkD=True, thread=True, qfield=False):
        ...
        try:
            while iterator.value():
                ...
                self.currentSet.append(
                    [
                        item.text(0),
                        self.treeWidget_layers.itemWidget(item, 1).currentText(),
                        self.treeWidget_layers.itemWidget(item, 2).currentText(),
                    ]
                )
                iterator += 1
        ...
        if thread:            
            threading.Thread(
                target=lambda: self.updateCompositionThread(qfield)
            ).start()
        else:  
    ...
    def itemClick(self, item, col):
        if item.checkState(0) == 2 and self.checkIfLayerIsInMoreGroups(
            ...
            self.showInfoDialogOnTop(
                self.tr("Layer ")
                + item.text(0)
                + self.tr(" is nested in two groups. Only parent group will be saved.")
            )
            ...
            for lay in composition["layers"]:
                if self.utils.removeUnacceptableChars(lay["title"]) in layerList:
                    try:
                        ...
                        self.layman.updateLayerStyle(
                            lay["title"], self.layman.laymanUsername
                        )  ## pokud je starší typ kompozice
                    try:
                        self.layman.stylesToUpdate.remove(
    ...
    def saveMapLayers(self):
        ...
        while iterator.value():
            ...
            if (
                ...
                and self.treeWidget_layers.itemWidget(item, 2).currentText()
                in ("Smazat", "Remove")
        ...
        if uniq:
            if len(layers) > 0:          
                ...
                if len(newLayers) > 0:                                   
                    self.layman.addLayerToComposition(
                        composition, layers, self.currentSet
                    )
            return True
        else:
    ...
    def setPermissionsUI(self, mapName):
        ...
        if not self.permissionsConnected:           
            self.pushButton_save_permissions.clicked.connect(
                lambda: self.progressBar_loader.show()
            )
            self.pushButton_save_permissions.clicked.connect(
                lambda: threading.Thread(
                    target=self.collectPermissionsAndSave,
                    args=(self.tabWidget, mapName),
                ).start()
            )
            self.pushButton_save_permissions.clicked.connect(
                lambda: threading.Thread(
                    target=self.updateQfieldPermissions, args=(self.tabWidget, mapName)
                ).start()
            )
        ...
        if self.comboBox_users.currentText() in allItems:
            if type == "read":
                if (
                    ...
                    self.showInfoDialogOnTop(
                        self.tr("This user already exists in the list!")
                    )
                    return False
            else: 
                if (
    ...
    def setMetadataUI(self):       
        ...
        self.pushButton_save.setEnabled(False)
        self.pushButton_back_3.clicked.connect(
        ...
        self.comboBox_epsg.addItems(
            [value.split(":")[1] for value in self.layman.supportedEPSG]
        )
        ...
        self.pushButton_range.setToolTip(
            self.tr("Gets spatial range information from wms capatibilies.")
        )
        self.pushButton_range_2.setToolTip(
            self.tr("Gets spatial range information from QGIS canvas.")
        )
    ...
    def modifyMeta(self):      
        ...
        coords = self.utils.tranformCoords(
            float(self.lineEdit_xmin.text().replace(",", ".")),
            float(self.lineEdit_xmax.text().replace(",", ".")),
            float(self.lineEdit_ymin.text().replace(",", ".")),
            float(self.lineEdit_ymax.text().replace(",", ".")),
            src,
            dest,
        )
        coords = self.utils.tranformCoords(
        ...
        center = tform.transform(
            QgsPointXY(
                self.layman.iface.mapCanvas().extent().center().x(),
                self.layman.iface.mapCanvas().extent().center().y(),
            )
        )
        ...
        if response.status_code == 200:
            self.utils.showQgisBar(
                [
                    " Metadata byla úspěšně upravena.",
                    " Map metadata was saved successfully.",
                ],
                Qgis.Success,
            )
        else:
            self.utils.showErr.emit(
                [" Metadata nebyla upravena.", " Map metadata was not saved."],
    ...
    def setExtentFromLayers(self):
        ...
        if xmin == None or xmax == None or ymin == None or ymax == None:
            self.showInfoDialogOnTop(
                self.tr(
                    "A record of the layers spatial extent for the selected composition was not found!"
                )
            )
        else:
    ...
    def copyCompositionUrl(self, composition=None):
        ...
        try: 
            ...
            info = str(e)
            allInfo = traceback.format_exception(
    ...
    def setNewUI(self): 
        ...
        self.pushButton_qfield.setEnabled(False)
        self.pushButton_copyUrl.setEnabled(False)
        self.pushButton_defaultExtent.clicked.connect(
            lambda: self.setDefaultExtent(ext)
        )
        self.setStyleSheet("#DialogBase {background: #f0f0f0 ;}") 
        if not self.pushButton_CreateCompositionConnected:                        
            self.pushButton_CreateComposition.clicked.connect(
                lambda: self.createComposition(
    ...
    def setExtent(self, it, col):
        ...
        ymax = ext.yMaximum()
        if (
            QgsProject.instance().crs().authid() == "EPSG:5514"
            and layer[0].crs().authid() == "EPSG:4326"
        ):
            ...
            ymax = max[1]
        if (
            QgsProject.instance().crs().authid() == "EPSG:4326"
            and layer[0].crs().authid() == "EPSG:5514"
        ):
    ...
    def checkNameCreateMap(self):
        ...
        if text != "":
            ...
            url = self.layman_api.get_map_file_url(
                self.layman.laymanUsername, str(text)
            )
            ...
            if not e:
                ...
                self.utils.showQgisBar(
                    [
                        " Kompozice s tímto jménem již existuje",
                        " Composition name already exists!",
                    ],
                    Qgis.Warning,
                )
    ...
    def createComposition(self, title, abstract, setCurrent=False):
        ...
        if title == "":
            ...
            if setCurrent:                
                ...
                url = self.layman_api.get_map_file_url(
                    self.layman.selectedWorkspace, name
                )
                r = self.utils.requestWrapper("GET", url, payload=None, files=None)
                data = r.json()        
                self.layman.instance = CurrentComposition(
                    self.URI,
                    name,
                    self.layman.selectedWorkspace,
                    self.utils.getAuthHeader(self.layman.authCfg),
                    self.layman.laymanUsername,
                )
    ...
    def setLayerPropertiesUI(self):             
        self.pushButton_back_4.clicked.connect(
            lambda: self.setStackWidget("main", False)
        )
        layerName = self.treeWidget_layers.selectedItems()[0].text(0)      
        self.label_name.setText(layerName)
        self.pushButton_save_props.clicked.connect(
            lambda: self.saveCompositionLayerChanges(layerName)
        )
        composition = self.layman.instance.getComposition()           
        for layer in composition["layers"]:
            if layer["title"] == layerName:
                ...
                self.label_max.setText(
                    "None"
                    if not layer["maxResolution"]
                    else "1:"
                    + str(self.utils.resolutionToScale(layer["maxResolution"]))
                )
                self.label_min.setText(
                    "1:"
                    + str(int(self.utils.resolutionToScale(layer["minResolution"])))
                )
                self.checkBox_visibility.setChecked(
                    True if layer["visibility"] else False
                )
    ...
    def saveCompositionLayerChanges(self, name):
        ...
        if response.status_code == 200:
            self.utils.showQgisBar(
                [" Změny byly uloženy.", " Changes was saved."], Qgis.Success
            )
        else:
            self.utils.showErr.emit(
                [" Změny nebyly uloženy.", " Changes was not saved."],
                "code: " + str(response.status_code),
                str(response.content),
                Qgis.Warning,
                "",
    ...
    def deleteCurrentMap(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if response.status_code == 200:
                self.utils.showQgisBar(
                    [
                        " Kompozice  " + composition["name"] + " byla úspešně smazána.",
                        " Composition  "
                        + composition["name"]
                ...
                self.utils.showErr.emit(
                    [
                        " Kompozice  " + composition["name"] + " nebyla smazána.",
                        " Composition  "
                        + composition["name"]
                        + " was not sucessfully deleted.",
                    ],
                    "code: " + str(response.status_code),
                    str(response.content),
                    Qgis.Warning,
    ...
    def loadCompositionFromData(self, data, filename):
        try:
            ...
            composition_name = data.get("name", os.path.splitext(os.path.basename(filename))[0])
            ...
            if isinstance(data, dict) and "layers" in data:
                ...
                print("DEBUG: Data is corrupted or missing layers, skipping layer loading")
                self.progressDone.emit()
    ...
    def loadLayerFromData(self, data, service, groupName=""):
        ...
        for x in range(len(data["layers"]) - 1, -1, -1):
            ...
            if className == "HSLayers.Layer.WMS" or className == "WMS":
                ...
                everyone = not self.layman.isAuthorized  # Použij authcfg pokud je uživatel autentifikovaný
                ...
                try:
                    if "type" in data["layers"][x]["protocol"]:
                        if (
                            ...
                            or data["layers"][x]["protocol"]["type"] == "hs.format.externalWFS"
                        ):
                            self.layman.loadWfs(
                                repairUrl,
                                layerName,
