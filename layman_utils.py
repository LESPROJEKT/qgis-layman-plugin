...
class LaymanUtils(QObject):
    ...
    @property
    def layman_api(self):
        if self._layman_api is None:
            if not self.URI:
                raise ValueError("Layman API URI is not set.")
            self._layman_api = LaymanAPI(self.URI)
        return self._layman_api
    ...
    def getDPI(self):
        return (
            self.iface.mainWindow().physicalDpiX()
            / self.iface.mainWindow().logicalDpiX()
        )
    ...
    def requestWrapper(
        ...
        try:
            ...
            if emitErr:
                self.showErr.emit(
                    ["Připojení není k dispozici", "Connection is not available"],
                    info,
                    str(info),
                    Qgis.Warning,
                    "",
        ...
        if emitErr:
            if response.status_code not in (200, 201):
                print(url)
                self.showErr.emit(
                    ["Požadavek nebyl úspěšný", "Request was not successfull"],
    ...
    def showMessageError(self, text, info, err, typ, url):
        ...
        def showDlg():
            ...
            self.dlgErr.pushButton_copyMsg.setStyleSheet(
                ...
                + self.fontSize
                + ";  text-decoration: none;   background: #72c02c;   padding: 6px;  border-radius: 50px;    display: inline-block; border: none;transition: all 0.4s ease 0s;"
            )
            self.dlgErr.pushButton_copyMsg.clicked.connect(copy_to_clipboard)
            self.dlgErr.plainTextEdit.setPlainText(
                text[0] + " - " + str(info)
                if self.locale == "cs"
                else text[1] + " - " + str(info)
            )
            self.dlgErr.show()
            ...
            if success[0] == True:
                ...
                if self.locale == "cs":
                    QMessageBox.information(
                        None,
                        "Message",
                        "Autorizace nebyla úspěšná! Zkuste to prosím znovu.",
                    )
                else:
                    QMessageBox.information(
                        None,
                        "Message",
                        "Autorization was not sucessfull! Please try it again.",
                    )
                return False
        else:
            return ""
    ...
    def showMessageBar(self, msg, state):
        if self.locale == "cs":
            self.iface.messageBar().pushWidget(
                self.iface.messageBar().createMessage("Layman:", msg[0]),
                state,
                duration=3,
            )
        else:
            self.iface.messageBar().pushWidget(
                self.iface.messageBar().createMessage("Layman:", msg[1]),
                state,
                duration=3,
            )
    ...
    def loadWms(
        ...
        urlWithParams = (
            ...
            + "&crs="
            + epsg
            + "&IgnoreReportedLayerExtents=1&dpiMode=7&featureCount=10&format=image/png&layers="
            + layerName
            + "&styles=&url="
            + url
        )
        ...
        try:
            if timeDimension != {}:
                if "values" in timeDimension["time"]:
                    ...
                    quri.setParam(
                        "timeDimensionExtent", str(timeDimension["time"]["values"])
                    )
                    ...
                    quri.setParam(
                        "timeDimensionExtent",
                        self.readDimFromCapatibilites(url, layerName),
                    )
                    quri.setParam("allowTemporalUpdates", "true")
                    quri.setParam("temporalSource", "provider")
        ...
        rlayer = QgsRasterLayer(
            str(quri.encodedUri(), "utf-8").replace("%26", "&").replace("%3D", "="),
            layerNameTitle,
            "wms",
        )
    ...
    def checkLayerOnLayman(self, layer_name, workspace, laymanUsername):
        if workspace:
            ...
            url = self.layman_api.get_layer_url(laymanUsername, layer_name)
        r = requests.get(url=url, headers=self.getAuthHeader(self.authCfg))
        try:
            data = r.json()

            if (
                data["wms"]["status"] == "NOT_AVAILABLE"
                or data["wms"]["status"] == "PENDING"
            ):
                return False
            else:
                return True
                ...
                if style[0] == 200:
                    if style[1] == "sld":
                        tempf = (
                            tempfile.gettempdir()
                            + os.sep
                            + self.removeUnacceptableChars(layerName)
                            + ".sld"
                        )
    ...
    def getStyle(self, layer_name, style=None, workspace=None):
        ...
        if self.selectedWorkspace:
            url = self.layman_api.get_layer_style_url(
                self.selectedWorkspace, self.removeUnacceptableChars(layer_name)
            )
            response = requests.get(url, headers=self.getAuthHeader(self.authCfg))
        else:
            url = self.layman_api.get_layer_style_url(
                self.laymanUsername, self.removeUnacceptableChars(layer_name)
            )
            response = requests.get(url, headers=self.getAuthHeader(self.authCfg))
        ...
        try:
            tempf = (
                tempfile.gettempdir()
                + os.sep
                + self.removeUnacceptableChars(layer_name)
                + suffix
            )
        except:
    ...
    def checkPublicationStatus(self, layer):
        url = self.layman_api.get_layer_url(self.laymanUsername, layer)
...
QPushButton::indicator {
    ...
    def checkWgsExtent(self, layer):
        ...
        extent = layer.extent()
        if (
            extent.xMaximum() > WgsXmax
            or extent.xMinimum() < WgsXmin
            or extent.yMaximum() > WgsYmax
            or extent.yMinimum() < WgsYmin
        ):
            return False
        else:
            return True
    ...
    def checkIfNotLocalLayer(self):
        ...
        layer_found = any(
            layer.dataProvider().name() not in accepted_data_providers
            for layer in layers
        )
        if layer_found:
            return True
    ...
    def compare_json_layers(self, schema1, schema2):
        ...
        if extra_layers1:
            print(
                f"Ve schématu jedna ubyly tyto vrstvy oproti schématu dva: {', '.join(extra_layers1)}"
            )
            return True
        if extra_layers2:
            print(
                f"Ve schématu jedna přibyly tyto vrstvy oproti schématu dva: {', '.join(extra_layers2)}"
            )
            return True
    ...
    def updateLayerAccessRights(self, filtered_layers):
        for layer in filtered_layers:
            ...
            response = requests.patch(
                url, data=data, headers=self.getAuthHeader(self.authCfg)
            )
            print(
                f"Update for layer '{name}': {response.status_code}, Response: {response.text}"
            )

    def removeAuthcfg(self, json_layers):
        layer_data = (
            json.loads(json_layers) if isinstance(json_layers, str) else json_layers
        )
        ...
        for layer in project.mapLayers().values():
            if layer.name() in layers_to_update:
                ...
                filtered_parts = [
                    part for part in parts if not part.startswith("authcfg=")
                ]
    ...
    def compareUpdates(self, qfieldUpdate, laymanUpdate):
        ...
        if updated_at1 != created_at1:
            ...
            return (
                "Informace: updated_at a created_at v prvním JSON objektu jsou stejné."
            )
    ...
    def compareLayers(qfieldFiles, laymanLayers, layersInProject):
        ...
        layer_dict2 = {
            layer["title"]: layer
            for layer in laymanLayers
            if layer["title"] in layersInProject
        }
        for layer1 in qfieldFiles:
            ...
            if name1.endswith(".gpkg"):
                ...
                last_modified1 = datetime.strptime(
                    last_modified1_str, "%d.%m.%Y %H:%M:%S UTC"
                )
                if base_name1 in layer_dict2:
                    layer2 = layer_dict2[base_name1]
                    updated_at2_str = layer2["updated_at"]
                    updated_at2 = datetime.fromisoformat(
                        updated_at2_str.replace("Z", "+00:00")
                    )
                    time_difference = updated_at2 - last_modified1
                    if time_difference >= timedelta(minutes=3):
                        print(
                            f"Informace: last_modified vrstvy '{name1}' je starší nebo rovno 3 minutám oproti updated_at vrstvy '{base_name1}'."
                        )
                    else:
                        print(
                            f"Informace: last_modified vrstvy '{name1}' není starší než 3 minuty oproti updated_at vrstvy '{base_name1}'."
                        )
                else:
                    print(
                        f"Informace: Vrstva s názvem '{base_name1}' nebyla nalezena ve druhém seznamu."
                    )
    ...
    def getLayersFromComposition(self, name):
        url = self.layman_api.get_map_file_url(self.laymanUsername, name)
    ...
    def estimateMbtilesSizeWmsLayer(
        self, active_layer, min_zoom=14, max_zoom=16, avg_tile_size_kb=10, dpi=96
    ):
    ...
    def estimateSizeFromScale(
        self, scale, base_scale=1885, base_size_mb=1.8, scale_multiplier=3
    ):
            ...
            if isinstance(layer, QgsRasterLayer):
                if layer.providerType() in ["wms", "xyz"]:
                    ...
                    if estimated_size > size_threshold_mb:
                        print(
                            f"Vrstva '{layer.name()}' má odhadovanou velikost {estimated_size:.2f} MB, což přesahuje limit {size_threshold_mb} MB."
                        )
    ...
    def export_layer_to_mbtiles(
        self, layer, output_directory, file_name, min_zoom=14, max_zoom=16, dpi=100
    ):
        extent = self.iface.mapCanvas().extent()
            ...
            if isinstance(layer, QgsRasterLayer):
                ...
                if provider_type in ["wms", "wmsprovider", "wms_client", "xyz"]:
                    ...
                    if output_format == "tiff":
                        ...
                        output_file = self.export_layer_to_mbtiles(
                            layer, temp_dir, file_name
                        )
                        print(output_file)
                    ...
                    if success:
                        ...
                        if output_format == "tiff":
                            ...
                            if tiff_layer.isValid():
                                ...
                                print(
                                    f"Chyba při vytváření vrstvy z TIFF pro {layer.name()}"
                                )
                        elif output_format == "mbtiles":
                            ...
                            if mbtiles_layer.isValid():
                                ...
                                print(
                                    f"Chyba při načítání MBTiles vrstvy pro {layer.name()}"
                                )
                    else:
                        print(
                            f"Chyba při exportu vrstvy {layer.name()} do {output_format.upper()}"
                        )
                else:
                    print(
                        f"Přeskakuji vrstvu {layer.name()} (poskytovatel: {provider_type})"
                    )
            else:
...
class CenterIconDelegate(QStyledItemDelegate):
    def paint(self, painter, option, index):
        ...
        if icon:
            ...
            iconX = round(
                option.rect.left() + (option.rect.width() - iconSize.width()) / 2
            )
            iconY = round(
